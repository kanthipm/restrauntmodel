...
service_rates <- c(6,6,6,6,6,6,6,9,9,9,9,9,9,9)
n_tables <- 5
...
filtered_waiting_times <- waiting_times[arrival_times_in_day >= 1020 & arrival_times_in_day <= 1320]

avg_wait_time_filtered <- if (length(filtered_waiting_times) > 0) mean(filtered_waiting_times) else 0
...
cat("Average Waiting Time from 5-10PM:", avg_wait_time_filtered, "minutes\n")
